...
void aardvark_i2c_write(uint8_t reg_addr, uint8_t data, void* private_data)
{
    ...
    int r = aa_i2c_write_ext(handle_, dev_addr, AA_I2C_NO_FLAGS,
                                 2, internal_buffer, &num_written);
    assert(r == AA_OK);
    assert(2 == num_written);
}

void aardvark_i2c_read(uint8_t reg_addr, uint8_t* data,
	size_t length, void* private_data)
{
	uint8_t dev_addr = *(uint8_t*)private_data;
    uint16_t num_written;
    uint16_t num_read;

    int r = aa_i2c_write_read(handle_, dev_addr, AA_I2C_NO_FLAGS,
                                 1, &reg_addr, &num_written,
